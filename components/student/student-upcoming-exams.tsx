...
export function StudentUpcomingExams() {
    ...
    >
      ...
      {hydrated && items.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-6 py-12 text-center">
          <p className="text-[15px] font-semibold text-slate-700">Yaklaşan deneme bulunmuyor</p>
          <p className="mt-1 text-[13px] text-slate-400">
            Kurum veya global takvime eklenen denemeler burada listelenir.
          </p>
        </div>
      ) : (
          ...
          {items.map((ex) => (
            ...
            >
              ...
              <div className="shrink-0 text-right">
                <p className="text-[15px] font-bold text-slate-900">{formatTrDate(ex.tarih)}</p>
