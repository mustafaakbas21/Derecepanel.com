...
import { useBookImport } from "@/lib/library/import/use-book-import";
import type { LibraryBook } from "@/lib/library/types";
import { cn } from "@/lib/utils";

type BookBulkImportDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported?: (records: LibraryBook[]) => void;
};

export function BookBulkImportDialog({
  open,
  onOpenChange,
  onImported,
}: BookBulkImportDialogProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [dragOver, setDragOver] = useState(false);

  const { downloadTemplate, importFile, isImporting, lastResult, clearResult } =
    useBookImport({ onImported });
